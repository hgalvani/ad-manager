...
from ldap3 import Server, Connection, ALL, NTLM, Tls, SUBTREE, core, extend
import sys
import ssl
...
def ad_modify_password(connexion, username, password):
  if isExist(username):
    # Get connection
    c = connexion
    dn = 'CN={},OU=Commerce,OU=SiteB,DC=anteverse,DC=com'.format(username)

    try:
      r = c.extend.microsoft.modify_password(dn, old_password=None, new_password='{}').format(password)
      print(r)
...
if __name__ == "__main__":
  ...
  # ad_modify_password(c, 'Alice DUPONT','AliceDupond2')
